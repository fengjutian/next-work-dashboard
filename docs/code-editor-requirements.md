...
### 3.6 Git 工作流增强（2026-08-01）

- 支持本地分支创建与切换，以及 Fetch、Pull、Push 和同步。
- 支持最近提交历史和完整 Commit Patch 查看。
- 支持 Stash 创建/列表，以及 Tag、Remote 的基础管理接口。
- Git 长操作通过进度事件显示当前状态；远端凭据缺失和冲突使用稳定错误码反馈。
- 所有 Git 参数通过 `execFile` 参数数组执行，关闭交互式凭据提示，不通过 Shell 拼接命令。
