...
当前实现状态：已提供默认关闭的影子模式。它按服务命名空间和模型过滤候选，记录 `>= 0.94` 与 `>= 0.97` 的候选分布，但始终继续请求真实模型，不会返回语义缓存答案。

精确缓存事件和语义相似度分桶会保留 90 天；设置页默认聚合最近 30 天的数据。事件仅包含事件类型、服务命名空间、模型、时间和相似度，不额外保存对话正文。
