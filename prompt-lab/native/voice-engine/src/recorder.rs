...
//! and emits an `audio.level` event every ~50 ms with the RMS level of the
//! last window.
//!
...
/// Sleep at most this long when the ring buffer is empty, so the consumer
/// can still exit promptly when capture is dropped.
const IDLE_POLL: Duration = Duration::from_millis(5);
/// How much audio we try to pop per iteration. 100 ms at 16 kHz.
const POP_CHUNK: usize = 1_600;
