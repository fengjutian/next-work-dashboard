...
export const ToolManagerDialog: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-2xl w-[520px] max-h-[80vh] flex flex-col">
        {/* 头部 */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-4">
          ...
          {tools.length === 0 && (
            <div className="text-center py-8">
              <Wrench className="h-10 w-10 text-zinc-300 dark:text-zinc-600 mx-auto mb-2" />
              <p className="text-xs text-zinc-400">暂无可用工具</p>
            </div>
          )}
        </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <code className="text-xs font-mono font-medium text-zinc-700 dark:text-zinc-300">
            {tool.name}
          </code>
        ...
        <p className="text-[10px] text-zinc-300 dark:text-zinc-500 mt-0.5">
          {paramHint}
        </p>
      </div>
      <button
