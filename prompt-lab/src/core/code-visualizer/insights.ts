import type { AnalysisEdge, ArchitectureFinding, ArchitectureHealthReport, ArchitectureRuleConfig, DatabaseAnalysisReport, RepositoryAnalysis, RepositorySourceFile, SmartInsight, SqlQueryArtifact } from './types';
import { DEFAULT_ARCHITECTURE_CONFIG, parseSqlStructure } from './advanced-analysis';

export function analyzeArchitectureHealth(result: RepositoryAnalysis, config: ArchitectureRuleConfig = DEFAULT_ARCHITECTURE_CONFIG): ArchitectureHealthReport {
  ...
  for (const edge of graph.edges) {
    ...
    if (source && target && config.forbidden.some((rule) => rule.from === source.kind && rule.to === target.kind)) findings.push({ id: `arch:layer:${edge.source}:${edge.target}`, rule: 'layer-violation', severity: 'warning', message: `${source.label} 不应直接依赖 ${target.label}`, nodes: [edge.source, edge.target], location: source.location });
  }
  for (const [source, edges] of outgoing) if (edges.length > config.maxFanOut) findings.push({ id: `arch:fanout:${source}`, rule: 'high-fan-out', severity: 'warning', message: `${nodeById.get(source)?.label ?? source} 依赖 ${edges.length} 个下游节点`, nodes: [source], location: nodeById.get(source)?.location });
  ...
  for (const [table, endpoints] of tableUsage) if (endpoints.length >= config.sharedTableThreshold) findings.push({ id: `arch:shared:${table}`, rule: 'shared-database', severity: 'warning', message: `${table} 被 ${endpoints.length} 个接口共享访问`, nodes: endpoints });
  const maxDepth = Math.max(0, ...result.endpoints.map((endpoint) => graphDepth(endpoint.edges)));
  if (maxDepth > config.maxDepth) findings.push({ id: 'arch:depth', rule: 'deep-chain', severity: 'warning', message: `最大调用深度达到 ${maxDepth}，超过配置 ${config.maxDepth}`, nodes: [] });
  const active = findings.filter((finding) => !config.ignoredRules.includes(finding.rule));
  return { score: Math.max(0, 100 - active.reduce((sum, finding) => sum + (finding.severity === 'error' ? 15 : 7), 0)), findings: active, metrics: { nodes: nodeById.size, edges: graph.edges.length, maxDepth, sharedTables: [...tableUsage.values()].filter((items) => items.length >= config.sharedTableThreshold).length } };
}
...
export function analyzeDatabaseQueries(result: RepositoryAnalysis, files: RepositorySourceFile[]): DatabaseAnalysisReport {
  ...
  for (const file of files) for (const match of file.content.matchAll(/(["'`]{1,3})(\s*(?:SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM)\b[\s\S]{3,2000}?)\1/gi)) {
    ...
    queries.push({ id: `sql:${file.path}:${line}`, operation, sql, tables, location: { file: file.path, line, snippet: sql.slice(0, 240) }, endpointIds, risks, structure: parseSqlStructure(sql) });
  }
