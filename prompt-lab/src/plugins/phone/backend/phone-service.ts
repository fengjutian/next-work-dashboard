...
function bindSocket(socket: WebSocket, expectedPeerId?: string): void {
  ...
  socket.on('open', () => socket.send(JSON.stringify(envelope('peer.hello', {}, expectedPeerId))));
...
async function handleWire(socket: WebSocket, raw: string, expectedPeerId?: string): Promise<string | undefined> {
  ...
  if (frame.type === 'peer.hello') return peer.id;
...
async function handleUpload(request: http.IncomingMessage, response: http.ServerResponse, peerId: string, fileId: string, name: string, expectedSha: string): Promise<void> {
  ...
  request.on('data', (chunk: Buffer) => { received += chunk.length; hash.update(chunk); }); request.pipe(output);
  output.on('finish', () => { const actual = hash.digest('hex'); if (expectedSha && actual !== expectedSha) { fs.rmSync(temp, { force: true }); response.writeHead(422).end(); return; } fs.renameSync(temp, target); receivedFiles.set(fileId, target); response.writeHead(201, { 'content-type': 'application/json' }); response.end(JSON.stringify({ path: target, size: received, sha256: actual })); });
  output.on('error', () => { fs.rmSync(temp, { force: true }); if (!response.headersSent) response.writeHead(500).end(); });
}
...
export async function stopPhoneService(): Promise<void> { if (discoveryTimer) clearInterval(discoveryTimer); if (pruneTimer) clearInterval(pruneTimer); for (const socket of sockets.values()) socket.close(); sockets.clear(); udp?.close(); wsServer?.close(); await new Promise<void>((resolve) => server?.close(() => resolve()) ?? resolve()); started = false; }
...
async function sendFiles(peerId: string): Promise<PhoneMessage[]> { const peer = peers.get(peerId); if (!peer?.trusted || peer.status !== 'online') throw new Error('对方不在线或尚未配对'); const result = await dialog.showOpenDialog({ properties: ['openFile', 'multiSelections'] }); if (result.canceled) return []; const messages: PhoneMessage[] = [];
  for (const source of result.filePaths) { const stats = fs.statSync(source); if (stats.size > MAX_FILE_BYTES) throw new Error(`${path.basename(source)} 超过 10 GB`); const sha256 = await new Promise<string>((resolve, reject) => { const hash = crypto.createHash('sha256'); fs.createReadStream(source).on('data', (chunk) => hash.update(chunk)).on('end', () => resolve(hash.digest('hex'))).on('error', reject); }); const fileId = uuid(); const auth = peerAuth(peerId); const uploadUrl = `http://${peer.host}:${peer.port}/phone/files/${fileId}?peerId=${encodeURIComponent(store.deviceId)}&name=${encodeURIComponent(path.basename(source))}&sha256=${sha256}`; const body = fs.createReadStream(source); const upload = await fetch(uploadUrl, { method: 'PUT', headers: { authorization: `Bearer ${auth}`, 'content-length': String(stats.size) }, body: body as unknown as BodyInit, duplex: 'half' } as RequestInit & { duplex: string }); if (!upload.ok) throw new Error(`文件发送失败 (${upload.status})`); const message: PhoneMessage = { id: uuid(), peerId, senderId: store.deviceId, recipientId: peerId, kind: 'file', file: { id: fileId, name: path.basename(source), size: stats.size, sha256, localPath: source }, createdAt: Date.now(), status: 'sent' }; persistMessage(message); sendWire(peerId, 'chat.file', { message }); messages.push(message); }
  return messages;
}
