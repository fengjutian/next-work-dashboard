...
import type { DiskDirectoryItem, DiskFilePreview, DiskScanEvent, DiskSpecialtyProbe, DiskSystemInfo, DiskUsnInfo } from '@/types/electron';
...
type PersistedScanResult = { id: string; root: string; savedAt: number; stats: { files: number; bytes: number; errors: number }; directories: DirectoryEntry[]; largest: FileEntry[]; extensions: Record<string, number>; duplicates: DuplicateGroup[] };
...
function Chart({ option, className }: { option: EChartsCoreOption; className: string }) {
  ...
  useEffect(() => {
    ...
    chart.on('click', (params) => { const data = typeof params.data === 'object' && params.data !== null ? params.data as { path?: string } : undefined; if (data?.path) window.dispatchEvent(new CustomEvent('disk-space:navigate', { detail: data.path })); });
...
export function DiskSpacePanel() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'browser' | 'analysis' | 'developer' | 'cleanup' | 'doctor'>('overview');
  const [system, setSystem] = useState<DiskSystemInfo | null>(null);
  const [diskHistory, setDiskHistory] = useState<DiskHistoryPoint[]>(() => { try { return JSON.parse(localStorage.getItem('disk-space.history') || '[]') as DiskHistoryPoint[]; } catch { return []; } });
  ...
  const [savedResults, setSavedResults] = useState<PersistedScanResult[]>(() => { try { return JSON.parse(localStorage.getItem('disk-space.results') || '[]') as PersistedScanResult[]; } catch { return []; } });
  ...
  const [specialtyProbes, setSpecialtyProbes] = useState<DiskSpecialtyProbe[]>([]); const [probing, setProbing] = useState(false);
  const [diagnosis, setDiagnosis] = useState(''); const [diagnosing, setDiagnosing] = useState(false);
  ...
  useEffect(() => { if (activeTab !== 'developer' || specialtyProbes.length > 0 || probing) return; setProbing(true); void window.electronAPI.diskSpace.probeSpecialties().then(setSpecialtyProbes).catch((cause) => setError(String(cause))).finally(() => setProbing(false)); }, [activeTab, probing, specialtyProbes.length]);
  useEffect(() => { if (!root) { setUsnInfo(null); setUsnDelta(null); return; } void window.electronAPI.diskSpace.usnInfo(root).then((info) => { setUsnInfo(info); if (info.supported && info.volume && info.nextUsn !== undefined) { const cursors = JSON.parse(localStorage.getItem('disk-space.usn-cursors') || '{}') as Record<string, { journalId?: number; nextUsn: number; recordedAt: number }>; const previous = cursors[info.volume]; setUsnDelta(previous && previous.journalId === info.journalId ? Math.max(0, info.nextUsn - previous.nextUsn) : null); cursors[info.volume] = { journalId: info.journalId, nextUsn: info.nextUsn, recordedAt: Date.now() }; localStorage.setItem('disk-space.usn-cursors', JSON.stringify(cursors)); } }).catch((cause) => setUsnInfo({ supported: false, error: String(cause) })); }, [root]);
  ...
  const choose = async () => {
    // 切换根目录前先 cancel 当前正在跑的扫描 —— 旧 child 进程会继续推事件污染新结果。
    const previousScanId = scanId.current;
    ...
    try {
      ...
      const saved = JSON.parse(localStorage.getItem('disk-space.last-result') || 'null') as null | { root: string; stats: typeof stats; directories: DirectoryEntry[]; largest: FileEntry[]; extensions: Record<string, number>; duplicates?: DuplicateGroup[] };
      if (saved && displayPath(saved.root).toLowerCase() === displayPath(chosen).toLowerCase()) {
        ...
        for (const file of saved.largest) largestTopRef.current.push(file);
        largestDirtyRef.current = false;
  ...
  const historyOption = useMemo<EChartsCoreOption>(() => { const diskNames = [...new Set(diskHistory.flatMap((point) => point.disks.map((disk) => disk.path)))]; return { color: ['#7c3aed', '#2563eb', '#0891b2', '#059669'], tooltip: { trigger: 'axis', formatter: (items: Array<{ seriesName: string; value: number }>) => items.map((item) => `${item.seriesName}：${formatBytes(item.value)}`).join('<br/>') }, legend: { top: 4, data: diskNames }, grid: { left: 16, right: 22, top: 42, bottom: 22, containLabel: true }, xAxis: { type: 'category', boundaryGap: false, data: diskHistory.map((point) => new Date(point.timestamp).toLocaleString([], { month: '2-digit', day: '2-digit', hour: '2-digit' })), axisLabel: { color: '#746075' } }, yAxis: { type: 'value', axisLabel: { color: '#746075', formatter: (value: number) => formatBytes(value) }, splitLine: { lineStyle: { color: 'rgba(127,127,127,.12)' } } }, series: diskNames.map((name) => ({ name, type: 'line', smooth: true, showSymbol: diskHistory.length < 12, areaStyle: { opacity: 0.06 }, data: diskHistory.map((point) => point.disks.find((disk) => disk.path === name)?.used ?? null) })) }; }, [diskHistory]);
  const visibleLargest = useMemo(() => { const custom = Number(customThresholdGb); const threshold = Number.isFinite(custom) && custom > 0 ? custom * 1024 ** 3 : largeFileThreshold; return largest.filter((file) => file.size >= threshold && (!largeFileExtension || file.extension.toLowerCase() === largeFileExtension)).sort((a, b) => largeFileSort === 'modified' ? b.modifiedAt - a.modifiedAt : largeFileSort === 'extension' ? a.extension.localeCompare(b.extension) : b.size - a.size); }, [customThresholdGb, largeFileExtension, largeFileSort, largeFileThreshold, largest]);
  const scannedExtensions = useMemo(() => [...new Set(largest.map((file) => file.extension).filter(Boolean))].sort(), [largest]);
  ...
  const cleanupAssessments = useMemo(() => cleanupItems.map((item) => { const name = displayPath(item.path).split(/[\\/]/).filter(Boolean).at(-1)?.toLowerCase() || ''; const lowRisk = /^(?:cache|caches|temp|tmp|logs?|__pycache__)$/.test(name); const mediumRisk = /^(?:target|dist|build)$/.test(name); return { ...item, risk: lowRisk ? '低风险' : mediumRisk ? '需要确认' : '仅建议检查', evidence: lowRisk ? '常见缓存、临时文件或日志目录' : mediumRisk ? '可由构建工具重新生成，但重新构建需要时间' : '可能包含项目依赖或用户仍在使用的数据' }; }), [cleanupItems]);
  const selectedDuplicateBytes = duplicates.flatMap((group) => group.files).filter((file) => selectedDuplicates.includes(file.path)).reduce((sum, file) => sum + file.size, 0);
  const generateDiagnosis = async () => { const evidence = { root, scanned: stats, largest: largest.slice(0, 10).map((file) => ({ path: displayPath(file.path), size: formatBytes(file.size) })), growth: directoryChanges.slice(0, 10).map((item) => ({ path: displayPath(item.path), change: `${item.change > 0 ? '+' : '-'}${formatBytes(Math.abs(item.change))}` })), cleanup: cleanupItems.slice(0, 10).map((item) => ({ path: displayPath(item.path), size: formatBytes(item.size), risk: '需要确认' })), duplicates: { groups: duplicates.length, reclaimable: formatBytes(duplicates.reduce((sum, group) => sum + group.size * (group.files.length - 1), 0)) } }; const local = `## 本地诊断\n\n- 扫描范围：${root || '尚未选择'}\n- 已扫描：${stats.files.toLocaleString()} 个文件，共 ${formatBytes(stats.bytes)}\n- 清理候选：${cleanupItems.length} 项，约 ${formatBytes(cleanupTotal)}（全部需要确认）\n- 重复文件：${duplicates.length} 组\n\n请先完成扫描；配置 AI 后可生成带证据的原因分析和清理顺序。`; if (!aiApi.apiKey || !aiApi.baseUrl || !aiApi.model) { setDiagnosis(local); return; } setDiagnosing(true); setDiagnosis(''); try { const provider = createOpenAIProvider({ apiKey: aiApi.apiKey, baseUrl: aiApi.baseUrl }); const messages = [{ role: 'system' as const, content: '你是本地磁盘诊断助手。只依据给定 JSON 证据分析，不得臆测。使用中文 Markdown，依次给出：容量结论、增长来源、按风险排序的清理建议、预计释放空间。每条结论必须引用具体路径和容量；清理候选均需人工确认，不建议删除系统目录。' }, { role: 'user' as const, content: JSON.stringify(evidence) }]; let text = ''; for await (const chunk of provider.chat(messages, { model: aiApi.model, temperature: 0.2, maxTokens: 1800, stream: true })) { if (chunk.delta) { text += chunk.delta; setDiagnosis(text); } } } catch (cause) { setDiagnosis(`${local}\n\n> AI 诊断失败：${cause instanceof Error ? cause.message : String(cause)}`); } finally { setDiagnosing(false); } };
  const exportScanReport = async () => { const content = `# 磁盘扫描报告\n\n- 扫描范围：${displayPath(root)}\n- 文件数量：${stats.files.toLocaleString()}\n- 扫描容量：${formatBytes(stats.bytes)}\n- 读取问题：${stats.errors}\n- 重复文件组：${duplicates.length}\n- 清理候选：${formatBytes(cleanupTotal)}\n\n## 最大文件\n\n${largest.slice(0, 30).map((file) => `- ${formatBytes(file.size)} · \`${displayPath(file.path)}\``).join('\n')}\n\n## 目录变化\n\n${directoryChanges.map((item) => `- ${item.change > 0 ? '+' : '-'}${formatBytes(Math.abs(item.change))} · \`${displayPath(item.path)}\``).join('\n')}`; await window.electronAPI.saveFile(content, `磁盘扫描报告-${new Date().toISOString().slice(0, 10)}.md`); };
  const directoryChanges = useMemo(() => { const matching = directorySnapshots.filter((snapshot) => displayPath(snapshot.root).toLowerCase() === displayPath(root).toLowerCase()).slice(-2); if (matching.length < 2) return []; const [previous, current] = matching; const before = new Map(previous.directories.map((item) => [displayPath(item.path).toLowerCase(), item])); const after = new Map(current.directories.map((item) => [displayPath(item.path).toLowerCase(), item])); return [...new Set([...before.keys(), ...after.keys()])].map((key) => { const oldItem = before.get(key); const newItem = after.get(key); return { path: newItem?.path ?? oldItem!.path, size: newItem?.size ?? 0, change: (newItem?.size ?? 0) - (oldItem?.size ?? 0) }; }).filter((item) => Math.abs(item.change) >= 1024 * 1024).sort((a, b) => Math.abs(b.change) - Math.abs(a.change)).slice(0, 20); }, [directorySnapshots, root]);
  ...
  return <div className="h-full min-h-0 overflow-y-auto bg-background p-5"><div className="mx-auto flex w-full max-w-[1700px] flex-col gap-4">
    ...
    <nav className="flex gap-1 overflow-x-auto rounded-xl border bg-card p-1 shadow-sm" aria-label="磁盘空间功能">{([['overview', '资源概览'], ['browser', '目录浏览'], ['analysis', '空间分析'], ['developer', '开发者空间'], ['cleanup', '清理建议'], ['doctor', '磁盘医生']] as const).map(([id, label]) => <button key={id} className={`shrink-0 rounded-lg px-5 py-2 text-sm transition-colors ${activeTab === id ? 'bg-primary text-primary-foreground shadow-sm' : 'text-muted-foreground hover:bg-accent hover:text-foreground'}`} onClick={() => setActiveTab(id)}>{label}</button>)}</nav>
    {root && <div className="flex items-center gap-2 text-xs text-muted-foreground"><span className={`rounded-full px-2 py-1 ${usnInfo?.supported ? 'bg-emerald-500/10 text-emerald-700' : 'bg-muted'}`}>{usnInfo?.supported ? `NTFS USN 已启用 · ${usnInfo.method === 'native' ? '原生 API' : '兼容模式'}` : '标准优化扫描'}</span>{usnInfo?.supported && usnInfo.volume && <span>{usnInfo.volume} Journal · Next USN {usnInfo.nextUsn?.toLocaleString()}</span>}{usnInfo?.error && <span title={usnInfo.error}>USN 不可用</span>}</div>}
    {usnInfo?.supported && usnDelta !== null && <div className="text-xs text-muted-foreground">相对上次记录，USN 游标前进 {usnDelta.toLocaleString()} 字节；Journal ID 变化时会自动放弃旧游标并执行完整扫描。</div>}
    ...
    <Modal open={errorsOpen} title={`扫描问题（${scanErrors.length}）`} footer={null} width="min(900px, 90vw)" onCancel={() => setErrorsOpen(false)}><div className="max-h-[65vh] overflow-auto">{scanErrors.length ? scanErrors.map((item, index) => <div key={`${item.path}-${index}`} className="border-b py-3"><p className="break-all font-mono text-xs">{displayPath(item.path)}</p><p className="mt-1 text-xs text-destructive">{item.message}</p></div>) : <EmptyState>没有记录到权限或读取问题</EmptyState>}</div></Modal>
    <Modal open={snapshotsOpen} title="扫描快照管理" footer={null} width="min(900px, 90vw)" onCancel={() => setSnapshotsOpen(false)}><div className="max-h-[65vh] overflow-auto">{directorySnapshots.map((snapshot) => <div key={`${snapshot.root}-${snapshot.timestamp}`} className="grid grid-cols-[minmax(0,1fr)_170px_80px] items-center gap-3 border-b py-3"><div className="min-w-0"><p className="truncate font-mono text-xs">{displayPath(snapshot.root)}</p><p className="text-xs text-muted-foreground">记录 {snapshot.directories.length} 个主要目录</p></div><span className="text-xs text-muted-foreground">{new Date(snapshot.timestamp).toLocaleString()}</span><button className="text-xs text-destructive hover:underline" onClick={() => setDirectorySnapshots((current) => { const next = current.filter((item) => item !== snapshot); localStorage.setItem('disk-space.directory-snapshots', JSON.stringify(next)); return next; })}>删除</button></div>)}</div></Modal>
    <Modal open={resultsOpen} title="扫描结果存档" footer={null} width="min(960px, 92vw)" onCancel={() => setResultsOpen(false)}><div className="max-h-[68vh] overflow-auto">{savedResults.length ? [...savedResults].reverse().map((saved) => <div key={saved.id} className="grid grid-cols-[minmax(0,1fr)_120px_170px_130px] items-center gap-3 border-b py-3"><div className="min-w-0"><p className="truncate font-mono text-xs">{displayPath(saved.root)}</p><p className="text-xs text-muted-foreground">{saved.stats.files.toLocaleString()} 个文件 · {formatBytes(saved.stats.bytes)}</p></div><span className="text-right text-xs">{saved.duplicates.length} 组重复</span><span className="text-xs text-muted-foreground">{new Date(saved.savedAt).toLocaleString()}</span><div className="flex justify-end gap-2"><button className="text-xs text-primary hover:underline" onClick={() => restoreSavedResult(saved)}>恢复</button><button className="text-xs text-destructive hover:underline" onClick={() => setSavedResults((current) => { const next = current.filter((item) => item.id !== saved.id); localStorage.setItem('disk-space.results', JSON.stringify(next)); return next; })}>删除</button></div></div>) : <EmptyState>暂无扫描结果存档</EmptyState>}</div></Modal>
    {activeTab === 'overview' && directorySnapshots.length > 0 && <div className="flex justify-end"><button className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent" onClick={() => setSnapshotsOpen(true)}>管理扫描快照</button></div>}
    {activeTab === 'overview' && savedResults.length > 0 && <div className="flex justify-end"><button className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent" onClick={() => setResultsOpen(true)}>查看扫描存档（{savedResults.length}）</button></div>}
    {activeTab === 'developer' && <section className="rounded-2xl border bg-card p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="font-medium">专项环境探测</h2><p className="text-xs text-muted-foreground">调用只读官方命令，不修改 Docker、WSL 或开发工具数据</p></div><button className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent disabled:opacity-50" disabled={probing} onClick={() => { setSpecialtyProbes([]); }}>{probing ? '探测中…' : '重新探测'}</button></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{specialtyProbes.map((probe) => <details key={probe.id} className="rounded-lg border p-4"><summary className="cursor-pointer text-sm font-medium">{probe.label}<span className={`ml-2 rounded-full px-2 py-0.5 text-xs ${probe.available ? 'bg-emerald-500/10 text-emerald-700' : 'bg-muted text-muted-foreground'}`}>{probe.available ? '已检测' : '未检测'}</span></summary><p className="mt-2 text-xs text-muted-foreground">{probe.summary}</p>{probe.details.length > 0 && <pre className="mt-3 max-h-52 overflow-auto whitespace-pre-wrap rounded-md bg-muted/50 p-3 font-mono text-xs">{probe.details.join('\n')}</pre>}</details>)}</div></section>}
    {activeTab === 'cleanup' && <section className="rounded-2xl border bg-card p-5 shadow-sm"><h2 className="font-medium">官方清理动作</h2><p className="mt-1 text-xs text-muted-foreground">每次执行都会显示系统确认框、实际命令和影响说明</p><div className="mt-4 grid gap-3 md:grid-cols-3">{([['docker-build-cache', 'Docker Build Cache', '需要确认'], ['npm-cache', 'npm 缓存', '低风险'], ['pnpm-store', 'pnpm Store', '低风险']] as const).map(([action, label, risk]) => <div key={action} className="rounded-lg border p-4"><div className="flex items-center justify-between"><span className="text-sm font-medium">{label}</span><span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-xs text-amber-700">{risk}</span></div><button className="mt-4 w-full rounded-md border px-3 py-2 text-sm hover:bg-accent" onClick={() => void window.electronAPI.diskSpace.runCleanup(action, root).then((result) => { if (result.success) setError(`${label} 清理完成`); }).catch((cause) => setError(String(cause)))}>查看影响并执行</button></div>)}</div></section>}
    {activeTab === 'cleanup' && cleanupAssessments.length > 0 && <section className="overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="border-b px-5 py-3"><h2 className="font-medium">候选风险与识别依据</h2><p className="text-xs text-muted-foreground">路径扫描仅用于建议；文件夹不会在此处直接删除</p></div>{cleanupAssessments.map((item) => <div key={item.path} className="grid grid-cols-[minmax(0,1fr)_110px_110px] items-center gap-3 border-b px-5 py-3"><div className="min-w-0"><p className="truncate font-mono text-xs" title={displayPath(item.path)}>{displayPath(item.path)}</p><p className="mt-1 text-xs text-muted-foreground">{item.evidence}</p></div><span className="text-right text-sm font-medium">{formatBytes(item.size)}</span><span className={`text-right text-xs font-medium ${item.risk === '低风险' ? 'text-emerald-700' : item.risk === '需要确认' ? 'text-amber-700' : 'text-red-700'}`}>{item.risk}</span></div>)}</section>}
    {activeTab === 'doctor' && <section className="rounded-2xl border bg-card p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-semibold">AI 磁盘医生</h2><p className="mt-1 text-sm text-muted-foreground">只发送容量、路径和变化摘要，不读取或上传文件内容</p></div><button className="rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground disabled:opacity-50" disabled={diagnosing || stats.files === 0} onClick={() => void generateDiagnosis()}>{diagnosing ? '正在诊断…' : '生成诊断报告'}</button></div><div className="mt-5 grid gap-3 sm:grid-cols-4"><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">扫描容量</p><p className="mt-1 font-semibold">{formatBytes(stats.bytes)}</p></div><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">增长来源</p><p className="mt-1 font-semibold">{directoryChanges.filter((item) => item.change > 0).length}</p></div><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">清理候选</p><p className="mt-1 font-semibold">{formatBytes(cleanupTotal)}</p></div><div className="rounded-lg bg-muted/40 p-3"><p className="text-xs text-muted-foreground">重复可释放</p><p className="mt-1 font-semibold">{formatBytes(duplicates.reduce((sum, group) => sum + group.size * (group.files.length - 1), 0))}</p></div></div><div className="mt-5 min-h-[300px] rounded-lg border bg-background p-5">{diagnosis ? <XMarkdown content={diagnosis} className="chat-markdown prose prose-sm max-w-none dark:prose-invert" /> : <EmptyState>{stats.files ? '点击“生成诊断报告”，分析结果将引用真实扫描证据' : '请先在空间分析中完成一次扫描'}</EmptyState>}</div></section>}
    {activeTab === 'doctor' && stats.files > 0 && <div className="flex justify-end gap-2"><button className="rounded-md border px-3 py-2 text-sm hover:bg-accent" onClick={() => void exportScanReport()}>导出扫描报告</button><button className="rounded-md border px-3 py-2 text-sm hover:bg-accent disabled:opacity-50" disabled={!diagnosis} onClick={() => void window.electronAPI.saveFile(diagnosis, `磁盘医生-${new Date().toISOString().slice(0, 10)}.md`)}>导出诊断 Markdown</button></div>}
    {running && <div className="flex justify-end gap-2"><button className="rounded-md border px-3 py-1.5 text-sm hover:bg-accent" onClick={async () => { const success = paused ? await window.electronAPI.diskSpace.resume(scanId.current) : await window.electronAPI.diskSpace.pause(scanId.current); if (success) setPaused(!paused); }}>{paused ? '继续扫描' : '暂停扫描'}</button><button className="rounded-md border border-destructive/40 px-3 py-1.5 text-sm text-destructive hover:bg-destructive/10" onClick={() => void window.electronAPI.diskSpace.cancel(scanId.current)}>停止扫描</button></div>}
    {running && <section className="rounded-2xl border bg-card p-4 shadow-sm"><div className="grid gap-4 sm:grid-cols-4"><div><p className="text-xs text-muted-foreground">已运行</p><p className="mt-1 font-semibold">{formatDuration(scanTelemetry.elapsedMs)}</p></div><div><p className="text-xs text-muted-foreground">扫描速度</p><p className="mt-1 font-semibold">{scanTelemetry.elapsedMs ? Math.round(scanTelemetry.files / (scanTelemetry.elapsedMs / 1000)).toLocaleString() : 0} 文件/秒</p></div><div><p className="text-xs text-muted-foreground">目录</p><p className="mt-1 font-semibold">{scanTelemetry.directories.toLocaleString()}</p></div><div><p className="text-xs text-muted-foreground">已读取</p><p className="mt-1 font-semibold">{formatBytes(scanTelemetry.bytes)}</p></div></div><div className="mt-3 flex items-center gap-2"><Loader2 className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate font-mono text-xs" title={displayPath(scanTelemetry.currentPath)}>{phase === 'hashing' ? '正在校验重复文件…' : displayPath(scanTelemetry.currentPath)}</span>{scanErrors.length > 0 && <button className="text-xs text-destructive underline" onClick={() => setErrorsOpen(true)}>查看 {scanErrors.length} 个问题</button>}</div></section>}
    {activeTab === 'analysis' && stats.files > 0 && <section className="flex flex-wrap items-end gap-3 rounded-xl border bg-card p-4 shadow-sm"><label className="grid gap-1 text-xs text-muted-foreground">自定义最小容量（GB）<input className="w-40 rounded-md border bg-background px-3 py-2 text-sm text-foreground" type="number" min="0" step="0.1" value={customThresholdGb} placeholder="例如 2.5" onChange={(event) => setCustomThresholdGb(event.target.value)} /></label><label className="grid gap-1 text-xs text-muted-foreground">文件类型<select className="w-40 rounded-md border bg-background px-3 py-2 text-sm text-foreground" value={largeFileExtension} onChange={(event) => setLargeFileExtension(event.target.value)}><option value="">全部类型</option>{scannedExtensions.map((extension) => <option key={extension} value={extension}>.{extension}</option>)}</select></label><button className="rounded-md border px-3 py-2 text-sm hover:bg-accent" onClick={() => { setCustomThresholdGb(''); setLargeFileExtension(''); setLargeFileThreshold(0); }}>重置筛选</button></section>}
    {activeTab === 'analysis' && stats.files > 0 && <div className="flex justify-end gap-1">{([['size', '按大小'], ['modified', '按修改时间'], ['extension', '按扩展名']] as const).map(([value, label]) => <button key={value} className={`rounded-md border px-3 py-1.5 text-xs ${largeFileSort === value ? 'bg-primary text-primary-foreground' : 'hover:bg-accent'}`} onClick={() => setLargeFileSort(value)}>{label}</button>)}</div>}
    {activeTab === 'analysis' && duplicates.length > 0 && <section className="overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="flex items-center justify-between gap-4 border-b px-5 py-3"><div><h2 className="font-medium">重复文件工作台</h2><p className="text-xs text-muted-foreground">完整哈希并逐字节复核；每组第一项建议保留</p></div><button className="rounded-md border border-destructive/40 px-3 py-2 text-sm text-destructive disabled:opacity-40" disabled={!scanId.current || selectedDuplicates.length === 0 || running} onClick={async () => { const result = await window.electronAPI.diskSpace.trash(scanId.current, selectedDuplicates); if (!result.success) return; const removed = new Set(result.trashed); setDuplicates((current) => current.map((group) => ({ ...group, files: group.files.filter((file) => !removed.has(file.path)) })).filter((group) => group.files.length > 1)); setSelectedDuplicates([]); }}>移入回收站（{selectedDuplicates.length} · {formatBytes(selectedDuplicateBytes)}）</button></div><div className="max-h-[520px] overflow-auto p-4">{duplicates.map((group) => <div key={group.groupId} className="mb-3 overflow-hidden rounded-lg border"><div className="bg-muted/40 px-3 py-2 text-xs text-muted-foreground">{group.files.length} 个相同文件 · 单个 {formatBytes(group.size)} · 最多释放 {formatBytes(group.size * (group.files.length - 1))}</div>{group.files.map((file, index) => <label key={file.path} className="grid grid-cols-[24px_minmax(0,1fr)_110px] items-center gap-2 border-t px-3 py-2 text-sm hover:bg-muted/30"><input type="checkbox" disabled={index === 0} checked={selectedDuplicates.includes(file.path)} onChange={() => setSelectedDuplicates((current) => current.includes(file.path) ? current.filter((path) => path !== file.path) : [...current, file.path])} /><span className="truncate font-mono text-xs" title={displayPath(file.path)}>{displayPath(file.path)}{index === 0 && <span className="ml-2 font-sans text-primary">建议保留</span>}</span><span className="text-right text-xs text-muted-foreground">{new Date(file.modifiedAt).toLocaleDateString()}</span></label>)}</div>)}</div></section>}
    {activeTab === 'overview' && (diskHistory.length > 0 || directorySnapshots.length > 0) && <section className="flex items-center justify-between rounded-xl border bg-card px-5 py-3 shadow-sm"><div className="text-sm"><span className="font-medium">本地历史数据</span><span className="ml-3 text-xs text-muted-foreground">磁盘快照 {diskHistory.length} · 目录快照 {directorySnapshots.length}</span></div><button className="rounded-md border border-destructive/40 px-3 py-1.5 text-xs text-destructive hover:bg-destructive/10" onClick={() => { localStorage.removeItem('disk-space.history'); localStorage.removeItem('disk-space.directory-snapshots'); setDiskHistory([]); setDirectorySnapshots([]); }}>清空历史</button></section>}
    {(activeTab === 'developer' || activeTab === 'cleanup') && stats.files > 0 && <section className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border bg-card p-4 shadow-sm"><p className="text-xs text-muted-foreground">识别容量</p><p className="mt-1 text-2xl font-semibold">{formatBytes(activeTab === 'developer' ? developerTotal : cleanupTotal)}</p></div><div className="rounded-xl border bg-card p-4 shadow-sm"><p className="text-xs text-muted-foreground">候选目录</p><p className="mt-1 text-2xl font-semibold">{(activeTab === 'developer' ? developerItems : cleanupItems).length}</p></div><div className="rounded-xl border bg-card p-4 shadow-sm"><p className="text-xs text-muted-foreground">风险策略</p><p className="mt-1 text-base font-semibold">{activeTab === 'cleanup' ? '全部需要人工确认' : '只读分析'}</p></div></section>}
    {activeTab === 'analysis' && stats.files > 0 && <section className="overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="flex items-center justify-between border-b px-5 py-3"><div><h2 className="font-medium">目录容量变化</h2><p className="text-xs text-muted-foreground">与同一目录上一次完整扫描比较</p></div><span className="text-xs text-muted-foreground">保留最近 20 次扫描</span></div>{directoryChanges.length ? <div className="max-h-[360px] overflow-auto">{directoryChanges.map((item) => <div key={item.path} className="grid grid-cols-[minmax(0,1fr)_120px_120px] items-center gap-3 border-b px-5 py-2.5 text-sm"><span className="truncate font-mono text-xs" title={displayPath(item.path)}>{displayPath(item.path)}</span><span className="text-right tabular-nums text-muted-foreground">当前 {formatBytes(item.size)}</span><span className={`text-right font-medium tabular-nums ${item.change > 0 ? 'text-red-600' : 'text-emerald-600'}`}>{item.change > 0 ? '+' : '−'}{formatBytes(Math.abs(item.change))}</span></div>)}</div> : <div className="px-5 py-10 text-center text-sm text-muted-foreground">完成同一目录的第二次扫描后，将显示具体增长来源</div>}</section>}
    {activeTab === 'analysis' && stats.files > 0 && <section className="max-h-[430px] overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="flex items-center justify-between gap-4 border-b px-5 py-3"><div><h2 className="font-medium">大文件分析</h2><p className="text-xs text-muted-foreground">共显示 {visibleLargest.length} 个文件</p></div><div className="flex gap-1 rounded-lg bg-muted p-1">{[[0, '全部'], [1024 ** 3, '> 1 GB'], [5 * 1024 ** 3, '> 5 GB'], [10 * 1024 ** 3, '> 10 GB']] .map(([value, label]) => <button key={label} className={`rounded-md px-3 py-1.5 text-xs ${largeFileThreshold === value ? 'bg-background font-medium shadow-sm' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setLargeFileThreshold(value as number)}>{label}</button>)}</div></div><div className="max-h-[360px] overflow-auto">{visibleLargest.length ? visibleLargest.map((file) => <button key={file.path} className="grid w-full grid-cols-[minmax(0,1fr)_110px_150px] items-center gap-3 border-b px-5 py-2.5 text-left text-sm hover:bg-muted/40" onClick={() => void openPreview({ name: displayPath(file.path).split(/[\\/]/).at(-1) || file.path, path: file.path, type: 'file', size: file.size, modifiedAt: file.modifiedAt, extension: file.extension })}><span className="truncate font-mono text-xs" title={displayPath(file.path)}>{displayPath(file.path)}</span><span className="text-right font-medium tabular-nums">{formatBytes(file.size)}</span><span className="text-right text-xs text-muted-foreground">{new Date(file.modifiedAt).toLocaleDateString()}</span></button>) : <div className="py-12 text-center text-sm text-muted-foreground">没有符合当前大小条件的文件</div>}</div></section>}
    {activeTab === 'overview' && <section className="h-[300px] overflow-hidden rounded-2xl border bg-card shadow-sm"><div className="flex items-center justify-between border-b px-5 py-3"><div><h2 className="font-medium">磁盘使用趋势</h2><p className="text-xs text-muted-foreground">每小时记录一次，最多保留最近 7 天</p></div><span className="text-xs text-muted-foreground">{diskHistory.length} 个快照</span></div><div className="h-[245px]">{diskHistory.length ? <Chart option={historyOption} className="h-full w-full" /> : <EmptyState>首次记录后将在这里显示磁盘变化</EmptyState>}</div></section>}
