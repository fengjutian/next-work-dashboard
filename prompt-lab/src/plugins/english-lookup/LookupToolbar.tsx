...
export function LookupToolbar(props: Props) {
  ...
  return <section className="mx-auto mb-5 w-full max-w-4xl rounded-b-xl border border-t-0 bg-card p-3 shadow-sm sm:p-4">
    {props.history.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-2"><span className="text-[11px] text-muted-foreground">最近查询</span>{props.history.slice(0, 5).map((item) => <button key={item.query} onClick={() => props.onHistory(item.query)} className="max-w-32 truncate rounded-full bg-muted px-2.5 py-1 text-[11px] hover:bg-accent">{item.query}</button>)}<button onClick={props.onClearHistory} className="text-[11px] text-muted-foreground hover:text-foreground">清空</button></div>}
    <form onSubmit={submit} className="flex flex-col gap-2 sm:flex-row"><div className="relative min-w-0 flex-1"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground"/><input ref={props.inputRef} autoFocus value={props.query} onChange={(event) => props.onQuery(event.target.value)} placeholder="输入英文单词、短语或句子" className="h-10 w-full rounded-lg border bg-background pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/30"/></div><Button disabled={props.loading} type="submit">{props.loading ? <Loader2 className="mr-2 h-4 w-4"/> : <Sparkles className="mr-2 h-4 w-4"/>}AI 查询</Button></form>
