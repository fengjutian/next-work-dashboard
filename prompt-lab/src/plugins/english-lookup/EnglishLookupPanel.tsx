...
Give 1-4 useful definitions, authentic examples, 3-8 collocations, 1-4 topics and 4-12 meaningful relations. Never invent a word. If the input is a sentence, identify its most useful target word or phrase and explain its meaning in that context. For an inflected form, use its dictionary headword and mention the inflection in memoryTip.`;
...
export function EnglishLookupPanel() {
  ...
  const lookup = useCallback(async (input = query) => {
    ...
    const cached = entries.find((item) => item.query?.toLocaleLowerCase('en-US') === normalizedQuery.toLocaleLowerCase('en-US') || item.word === word);
    ...
    try {
      ...
      const chunks: string[] = [];
      for await (const chunk of provider.chat([{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: `Look up this English input and preserve its context: ${normalizedQuery}` }], { model: aiApi.model, temperature: 0.2, maxTokens: 1800, stream: true, signal: controller.signal })) if (chunk.delta) chunks.push(chunk.delta);
      const next = { ...parseLookupResponse(chunks.join(''), word), query: normalizedQuery, context: normalizedQuery.includes(' ') ? normalizedQuery : undefined };
  ...
  const rateReview = (entry: WordEntry, rating: 'forgot' | 'hard' | 'known') => setEntries((current) => current.map((item) => item.id === entry.id ? reviewEntry(item, rating) : item));
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    ...
    <main className="min-h-0 flex-1 overflow-auto p-6"><div className="mx-auto max-w-6xl">
      ...
      {tab === 'review' && (dueEntries.length ? <div className="mx-auto max-w-2xl space-y-4">{dueEntries.map((entry) => <article key={entry.id} className="rounded-xl border bg-card p-6"><h2 className="text-2xl font-semibold">{entry.word}</h2><p className="mt-1 text-sm text-muted-foreground">{entry.phonetic} · {entry.partOfSpeech}</p><p className="mt-5 text-sm">{entry.definitions[0]?.meaning}</p><div className="mt-5 flex gap-2 border-t pt-4"><Button variant="outline" onClick={() => rateReview(entry, 'forgot')}>忘记了</Button><Button variant="outline" onClick={() => rateReview(entry, 'hard')}>有点模糊</Button><Button onClick={() => rateReview(entry, 'known')}>认识</Button></div></article>)}</div> : <Empty text="今天没有待复习单词。"/>)}
