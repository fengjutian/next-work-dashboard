...
export const StyleImagePanel: React.FC = () => {
  ...
  const refreshLibrary = useCallback(() => {
    const next = listImages().map((item) => ({ id: item.id, dataUrl: URL.createObjectURL(item.image), prompt: item.prompt, style: item.style, provider: item.provider, model: item.model, aspectRatio: item.aspectRatio, size: item.size, createdAt: item.createdAt }));
    setResults((current) => {
  ...
  return <div className="flex h-full min-h-0 bg-background text-foreground">
    ...
    <section className="w-[360px] shrink-0 overflow-y-auto border-r p-5">
      ...
      {provider === 'minimax' ? <label className="mb-4 grid gap-1 text-xs"><input type="url" value={miniMaxReferenceUrl} onChange={(event) => setMiniMaxReferenceUrl(event.target.value)} className="h-9 rounded-md border bg-background px-3" placeholder="https://example.com/character.jpg" /><span className="text-[10px] text-muted-foreground">MiniMax 人物主体参考需要可公网访问的 HTTPS 图片链接。</span></label>
    ...
    <main className="min-w-0 flex-1 overflow-y-auto p-6">{selectedImage ? <article className="mx-auto max-w-4xl overflow-hidden rounded-xl border bg-card shadow-sm"><div className="bg-[linear-gradient(45deg,#eee_25%,transparent_25%),linear-gradient(-45deg,#eee_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#eee_75%),linear-gradient(-45deg,transparent_75%,#eee_75%)] bg-[length:20px_20px]"><img className="max-h-[calc(100vh-240px)] min-h-80 w-full object-contain" src={selectedImage.dataUrl} alt="当前生成图片" /></div><div className="p-4"><details><summary className="cursor-pointer text-xs font-medium">查看完整提示词</summary><p className="mt-2 whitespace-pre-wrap rounded-md bg-muted/40 p-3 text-xs leading-5 text-muted-foreground">{selectedImage.prompt}</p></details><p className="mt-2 text-[10px] text-muted-foreground">{selectedImage.model} · {selectedImage.aspectRatio} · {(selectedImage.size / 1024 / 1024).toFixed(1)} MB · {new Date(selectedImage.createdAt).toLocaleString()}</p><div className="mt-3 flex flex-wrap justify-end gap-2"><Button size="sm" variant="outline" onClick={() => void copyPrompt(selectedImage)}><Copy className="mr-1 h-4 w-4" />复制提示词</Button><Button size="sm" variant="outline" onClick={() => reusePrompt(selectedImage)}><Sparkles className="mr-1 h-4 w-4" />再次使用</Button><Button size="sm" variant="outline" onClick={() => download(selectedImage)}><Download className="mr-1 h-4 w-4" />下载</Button><Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" title="从本地数据库删除" onClick={() => void removeImage(selectedImage)}><Trash2 className="h-4 w-4" /></Button></div></div></article> : <div className="flex h-full min-h-80 flex-col items-center justify-center text-muted-foreground"><Image className="mb-4 h-16 w-16 opacity-20" /><p className="font-medium">生成结果会显示在这里</p><p className="mt-1 text-sm">生成后会自动保存图片和提示词到本地 SQLite</p></div>}</main>
    <aside className="w-72 shrink-0 overflow-y-auto border-l bg-muted/10 p-4"><div className="sticky top-0 z-10 mb-3 bg-background/95 pb-2 backdrop-blur"><h2 className="text-sm font-semibold">历史图片</h2><p className="text-[11px] text-muted-foreground">本地已保存 {results.length} 张</p></div><div className="space-y-3">{results.map((item, index) => <button type="button" key={item.id} onClick={() => setSelectedImageId(item.id)} className={`block w-full overflow-hidden rounded-xl border bg-card text-left shadow-sm transition hover:border-primary/60 ${selectedImage?.id === item.id ? 'border-primary ring-2 ring-primary/15' : ''}`}><img className="aspect-[4/3] w-full bg-muted object-cover" src={item.dataUrl} alt={`历史图片 ${index + 1}`} /><span className="block p-2"><span className="block truncate text-xs font-medium">{item.prompt || `生成图片 ${index + 1}`}</span><span className="mt-1 block text-[10px] text-muted-foreground">{item.model} · {item.aspectRatio}</span><span className="block text-[10px] text-muted-foreground">{new Date(item.createdAt).toLocaleString()}</span></span></button>)}</div>{!results.length && <div className="grid min-h-48 place-items-center rounded-xl border border-dashed px-4 text-center text-xs text-muted-foreground">生成过的图片会以卡片形式显示在这里</div>}</aside>
  </div>;
};
