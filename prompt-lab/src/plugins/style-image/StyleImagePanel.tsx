...
export const StyleImagePanel: React.FC = () => {
  ...
  return <div className="flex h-full min-h-0 bg-background text-foreground">
    ...
    <main className="min-w-0 flex-1 overflow-y-auto p-6">{results.length ? <><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-semibold">本地图片库</h2><p className="text-xs text-muted-foreground">已保存到 SQLite · {results.length} 张</p></div></div><div className="grid grid-cols-1 gap-5 xl:grid-cols-2">{results.map((item, index) => <article key={item.id} className="overflow-hidden rounded-xl border bg-card shadow-sm"><div className="relative bg-[linear-gradient(45deg,#eee_25%,transparent_25%),linear-gradient(-45deg,#eee_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#eee_75%),linear-gradient(-45deg,transparent_75%,#eee_75%)] bg-[length:20px_20px]"><img className="max-h-[620px] w-full object-contain" src={item.dataUrl} alt={`生成结果 ${index + 1}`} /></div><div className="flex items-start gap-3 p-3"><div className="min-w-0 flex-1"><p className="line-clamp-2 text-xs text-muted-foreground">{item.prompt}</p><p className="mt-1 text-[10px] text-muted-foreground">{item.model} · {item.aspectRatio} · {(item.size / 1024 / 1024).toFixed(1)} MB · {new Date(item.createdAt).toLocaleString()}</p></div><Button size="sm" variant="outline" onClick={() => download(item)}><Download className="mr-1 h-4 w-4" />下载</Button><Button size="sm" variant="outline" className="text-red-600 hover:text-red-700" title="从本地数据库删除" onClick={() => void removeImage(item)}><Trash2 className="h-4 w-4" /></Button></div></article>)}</div></> : <div className="flex h-full min-h-80 flex-col items-center justify-center text-muted-foreground"><Image className="mb-4 h-16 w-16 opacity-20" /><p className="font-medium">生成结果会显示在这里</p><p className="mt-1 text-sm">生成后会自动保存到本地 SQLite</p></div>}</main>
  </div>;
};
