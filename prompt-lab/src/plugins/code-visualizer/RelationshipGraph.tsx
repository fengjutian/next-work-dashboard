...
export function RelationshipGraph({ endpoint, onOpenSource }: { endpoint: ApiEndpoint; onOpenSource: (node: AnalysisNode) => void }): JSX.Element {
  ...
  useEffect(() => {
    ...
    const cy = cytoscape({
      ...
      style: [
        { selector: 'node', style: { 'background-color': (element) => COLORS[element.data('kind') as AnalysisNode['kind']], label: 'data(label)', color: '#f8fafc', 'font-size': 11, 'text-wrap': 'ellipsis', 'text-max-width': 130, 'text-valign': 'center', width: 170, height: 44, shape: 'round-rectangle', 'border-width': 2, 'border-color': '#ffffff', 'border-opacity': 0.18 } as any },
        { selector: 'node:selected', style: { 'border-width': 3, 'border-color': '#ffffff', 'border-opacity': 0.9 } as any },
        { selector: 'edge', style: { width: 1.5, 'line-color': '#94a3b8', 'target-arrow-color': '#94a3b8', 'target-arrow-shape': 'triangle', 'curve-style': 'bezier', label: 'data(label)', 'font-size': 9, color: '#64748b', 'text-background-color': '#ffffff', 'text-background-opacity': 0.85, 'text-background-padding': 2, 'line-style': 'solid' } as any },
        { selector: 'edge[confidence = "inferred"]', style: { 'line-style': 'dashed', opacity: 0.72 } as any },
      ],
