...
import type { AnalysisNode, ApiEndpoint, CodeVisualizerProjectHistory, CodeVisualizerScanSnapshot, CodeVisualizerSnapshotDiff, RepositoryAnalysis } from '../../core/code-visualizer';
...
export function CodeVisualizerPanel(): JSX.Element {
  ...
  return <div className="flex h-full min-h-0 bg-background text-foreground">
    ...
    <Modal open={architectureOpen} onCancel={() => setArchitectureOpen(false)} footer={null} width="92vw" title={`全局架构图 · ${result?.endpoints.length ?? 0} 个接口`} destroyOnClose>{result?.globalGraph ? <RelationshipGraph graph={result.globalGraph} onOpenSource={openSource}/> : <Empty description="重新扫描后生成全局架构图"/>}</Modal>
