...
export async function createDocxBase64(book: PublicationBook): Promise<string> {
  ...
  const body = [
    ...
    ...book.chapters.flatMap((chapter) => [
      p(chapter.title, "Heading1"),
      ...paragraphs(chapter.markdown).map((text) => p(text)),
    ]),
  ].join("");
  zip.file(
    "word/document.xml",
...
export async function createPdfBase64(book: PublicationBook): Promise<string> {
  ...
  const pages = [
    { title: book.title, lines: [book.author, book.description ?? ""] },
    ...book.chapters.flatMap((chapter) => {
      const lines = paragraphs(chapter.markdown).flatMap((item) => {
        const result: string[] = [];
        for (let index = 0; index < item.length; index += 30)
          result.push(item.slice(index, index + 30));
        return result;
      });
      const chunks: Array<{ title: string; lines: string[] }> = [];
      for (let index = 0; index < lines.length; index += 21)
        chunks.push({
          title: index ? `${chapter.title}（续）` : chapter.title,
          lines: lines.slice(index, index + 21),
        });
      return chunks;
    }),
  ];
  for (const page of pages) {
    ...
    if (!context) throw new Error("当前环境无法生成 PDF");
    context.fillStyle = "#fffdfa";
    context.fillRect(0, 0, width, height);
    context.fillStyle = "#251f22";
    context.font = "bold 52px serif";
    context.fillText(page.title, 120, 160);
    context.font = "32px serif";
    page.lines.forEach((line, index) =>
      context.fillText(line, 120, 280 + index * 62),
    );
  ...
  images.forEach((image, index) => {
    ...
    objects[pageId] = ascii(
      `<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /XObject << /Im0 ${pageId + 1} 0 R >> >> /Contents ${pageId + 2} 0 R >>`,
    );
    objects[pageId + 1] = concat(
      ascii(
...
export async function createEpubBase64(book: PublicationBook): Promise<string> {
  ...
  zip.file(
    "OEBPS/style.css",
    "body{font-family:serif;line-height:1.9;margin:8%;color:#211b20}h1{page-break-before:always}p{text-indent:2em}",
  );
  book.chapters.forEach((chapter, index) =>
    zip.file(
      `OEBPS/chapter-${index + 1}.xhtml`,
      `<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml"><head><title>${xml(chapter.title)}</title><link rel="stylesheet" href="style.css"/></head><body><h1>${xml(chapter.title)}</h1>${paragraphs(
        chapter.markdown,
      )
        .map((text) => `<p>${xml(text)}</p>`)
        .join("")}</body></html>`,
    ),
  );
  const nav = book.chapters
    .map(
      (chapter, index) =>
  ...
  zip.file(
    "OEBPS/nav.xhtml",
    `<?xml version="1.0" encoding="utf-8"?><html xmlns="http://www.w3.org/1999/xhtml" xmlns:epub="http://www.idpf.org/2007/ops"><body><nav epub:type="toc"><h1>目录</h1><ol>${nav}</ol></nav></body></html>`,
  );
