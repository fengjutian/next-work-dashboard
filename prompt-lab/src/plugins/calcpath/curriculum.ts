...
export const KNOWLEDGE_NODES: KnowledgeNode[] = [
  { id: 'algebra', title: '代数运算', description: '式的变形、因式分解与方程是进入极限计算的工具地基。', stage: 1, difficulty: 1, estimatedMinutes: 25, prerequisites: [], skills: ['algebra-manipulation'] },
  { id: 'functions', title: '函数与图像', description: '高中函数语言把代数关系变成变化关系，是微积分的起点。', stage: 1, difficulty: 1, estimatedMinutes: 25, prerequisites: [], skills: ['function-evaluation'] },
  { id: 'trigonometry', title: '三角与指数对数', description: '周期、增长和反函数为大学微积分提供核心函数素材。', stage: 1, difficulty: 2, estimatedMinutes: 35, prerequisites: [], skills: ['function-families'] },
  { id: 'limits', title: '极限与连续', description: '把高中函数的静态取值升级为无限趋近的动态过程。', stage: 2, difficulty: 2, estimatedMinutes: 35, prerequisites: ['functions'], skills: ['limit-evaluation'] },
...
export const PROBLEMS: Problem[] = [
  { id: 'alg-1', knowledgeId: 'algebra', skillId: 'algebra-manipulation', difficulty: 1, type: 'calculation', question: '将 x² − 4 因式分解。', answers: ['(x-2)(x+2)', '(x+2)(x-2)'], solution: '平方差公式 a²−b²=(a−b)(a+b)，所以 x²−4=(x−2)(x+2)。', hints: ['把 4 看作 2²。'] },
...
export const BRIDGE_CONTEXT: Record<string, { from: string[]; bridge: string; destination: string }> = {
  algebra: { from: ['平方差', '式的变形'], bridge: '消去 0/0 的表象', destination: '极限计算' },
  functions: { from: ['函数值', '定义域', '图像'], bridge: '把静态关系看成变化过程', destination: '极限与导数' },
  limits: { from: ['函数图像', '代数变形'], bridge: '无限趋近', destination: '导数定义' },
  derivatives: { from: ['斜率', '函数', '极限'], bridge: '平均变化率 → 瞬时变化率', destination: '优化与微分方程' },
  'chain-rule': { from: ['复合函数', '代数代换'], bridge: '逐层追踪变化', destination: '复杂模型求导' },
  integrals: { from: ['面积', '数列求和', '导数'], bridge: '微小量的无限累积', destination: '面积、体积与概率' },
};
