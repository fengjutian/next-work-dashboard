  ...
  return <div className="flex h-full min-h-0 bg-[#f7f8f4] text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    ...
    <main className="min-w-0 flex-1 overflow-auto p-5 md:p-8">
      {view === 'home' && <div className="mx-auto max-w-6xl">
        <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">你的数学能力地形</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">从高中地基，走向大学微积分</h1><p className="mt-2 text-sm text-slate-500">通过提问、观察、推导和 AI 对话理解知识，练习只用于最后确认。</p></div><button onClick={() => openLesson(getNextProblem(state).knowledgeId)} className="flex items-center gap-2 rounded-xl bg-[#d88b3d] px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[#c77b30]">继续引导课 <ArrowRight className="h-4 w-4" /></button></div>
        ...
        <section className="mt-7 rounded-3xl bg-[#e7efe7] p-6 dark:bg-emerald-950/40"><div className="text-xs font-semibold uppercase tracking-widest text-emerald-800 dark:text-emerald-300">今日引导主题</div><div className="mt-3 flex flex-wrap items-center justify-between gap-4"><div><h2 className="text-xl font-semibold">高中函数语言 → 无限趋近 → 极限与导数</h2><p className="mt-2 text-sm text-slate-600 dark:text-slate-400">先形成直觉，再操作图形、建立概念并完成推导；最后只用一道题确认理解。</p></div><button onClick={() => openLesson(getNextProblem(state).knowledgeId)} className="rounded-xl border border-emerald-900/15 bg-white px-4 py-2 text-sm font-medium dark:bg-slate-900">进入课堂</button></div></section>
      ...
      {view === 'practice' && <div className="mx-auto max-w-5xl"><button className="text-sm text-slate-500 hover:text-slate-800" onClick={() => openLesson(problem.knowledgeId)}>← 回到本节讲解</button><div className="mt-5"><LearningLandscape state={state} compact activeId={problem.knowledgeId} /></div><div className="mt-5 grid gap-5 lg:grid-cols-[1fr_270px]"><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex items-center justify-between text-xs text-slate-500"><span>{currentNode?.title}</span><span>理解检查 · 难度 {problem.difficulty}/5</span></div><h1 className="mt-6 text-2xl font-semibold leading-relaxed">{problem.question}</h1><input autoFocus value={answer} onChange={(event) => setAnswer(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && submit()} disabled={!!feedback} placeholder="输入你的推理结果" className="mt-7 w-full rounded-xl border border-slate-300 bg-transparent px-4 py-3 outline-none focus:border-emerald-600 dark:border-slate-700" />
        ...
        {feedback && <div className={`mt-5 rounded-2xl p-5 ${feedback.correct ? 'bg-emerald-50 text-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-100' : 'bg-rose-50 text-rose-900 dark:bg-rose-950/40 dark:text-rose-100'}`}><div className="flex items-center gap-2 font-semibold"><CheckCircle className="h-5 w-5" />{feedback.correct ? '理解已经连上了' : '回到讲解补一座桥'}</div><p className="mt-2 text-sm leading-6">{feedback.text}</p><button onClick={() => openLesson(problem.knowledgeId)} className="mt-4 rounded-xl bg-white px-4 py-2 text-sm font-medium text-slate-900 shadow-sm">回到本节课堂</button></div>}
