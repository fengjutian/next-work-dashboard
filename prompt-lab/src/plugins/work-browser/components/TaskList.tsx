 ...
 */
import { Card, List, Tag, Space, Typography, Empty, Button, Tooltip, Progress, Drawer, Steps, Select, Input, message } from 'antd';
...
export function TaskList({ workspaceId }: TaskListProps) {
  const { tasks, loading, refresh, setStatus, updateStep, createFromTemplate } = useTasks(workspaceId as WorkspaceId);
            ...
            {grouped.map((g) => g.items.length > 0 && (
              <div key={g.status}>
                ...
                <List
                  size="small"
                  dataSource={g.items}
                  renderItem={(t) => <TaskCard task={t} onOpen={() => setActiveTask(t)} />}
                />
              </div>
            ))}
          </Space>
...
function TaskDetail({ task, onSetStatus, onUpdateStep, onRefresh }: {
  task: Task;
