...
export const CodeExtractControls: React.FC<{ onExtract: (graph: GraphData) => void }> = ({ onExtract }) => {
  ...
  const handleExtract = useCallback(async () => {
    ...
    try {
      ...
      const graph = extractCodeGraph(documents);
      onExtract(graph);
  ...
  return <button className="w-full h-7 flex items-center justify-center gap-1 rounded border border-input text-xs hover:bg-accent disabled:opacity-50" disabled={extracting} onClick={() => void handleExtract()}>
