...
import { computeTextDiffHunks, type TextDiffHunk } from '@/lib/text-diff';
import { computeTextDiffHunksAsync } from '@/lib/text-diff-client';
...
export function useTextDiffHunks(original: string, modified: string): TextDiffState {
  ...
  useEffect(() => {
    ...
    void computeTextDiffHunksAsync(original, modified, controller.signal).then(
