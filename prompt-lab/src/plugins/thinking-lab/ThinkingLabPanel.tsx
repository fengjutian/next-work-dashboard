...
import type { FrameworkResult, ThinkingFrameworkId, ThinkingRun } from './thinking-types';
...
export function ThinkingLabPanel() {
  ...
  const execute = useCallback(async () => {
    ...
    if (!controller.signal.aborted && completed.length) {
      ...
      try {
        const merged = await synthesizeAnalyses(question.trim(), completed, ai, controller.signal, setSynthesis);
        setSynthesis(merged);
        const run: ThinkingRun = {
          id: crypto.randomUUID(), question: question.trim(), context: context.trim(), frameworkIds: selected,
          results: completed, synthesis: merged, model: ai.model, createdAt: Date.now(),
        };
        saveThinkingRun(run);
  ...
  }, [aiApi, configured, context, question, selected, updateResult]);
          ...
          {!!results.length && <div className="grid gap-4 lg:grid-cols-2">
            {results.map((result) => {
              ...
              return <article key={result.frameworkId} className="min-w-0 rounded-xl border bg-card p-4">
                <div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">{meta?.name}</h3><span className="text-xs text-muted-foreground">{result.status === 'running' ? <span className="flex items-center gap-1"><Loader2 className="h-3 w-3" />分析中</span> : result.status === 'done' ? '完成' : result.status === 'failed' ? '失败' : '等待'}</span></div>
