...
export const LyricStudioPanel: React.FC = () => {
  ...
  useEffect(() => {
    const closeTopLayer = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || loading || structureLoading || promptOptimizing) return;
  ...
  }, [generationCandidate, loading, promptOptimizing, showOverwriteConfirm, showStructureGenerator, structureLoading]);
  useEffect(() => {
  ...
  const handleGenerateStructure = async (continueWithLyrics = false) => { if (!structurePrompt.trim()) { setMessage('请先输入歌曲结构提示词'); return; } if (!aiApi.apiKey || !aiApi.baseUrl || !aiApi.model) { setMessage('请先在设置中配置 AI 服务'); return; } setStructureLoading(true); try { const generatedStructure = await generateSongStructure(aiApi.apiKey, aiApi.baseUrl, aiApi.model, structurePrompt.trim()); const { sections } = generatedStructure; const generatedTitle = generatedStructure.title || project.title; saveRevision('生成歌曲结构前'); if (!continueWithLyrics) { patchProject({ title: generatedTitle, sections }); setActiveId(sections[0]?.id ?? ''); setShowStructureGenerator(false); setMessage(`已创建《${generatedTitle}》的 ${sections.length} 个空白段落；点击顶部歌名可继续修改`); return; } setMessage(`结构生成成功，正在为《${generatedTitle}》的 ${sections.length} 个段落填写歌词…`); const nextProject = { ...project, title: generatedTitle, sections }; const controller = new AbortController(); const result = await withRequestTimeout(generateLyrics(aiApi.apiKey, aiApi.baseUrl, aiApi.model, nextProject, controller.signal, (stage) => setMessage(stage)), controller); const filledSections = sections.map((section, index) => { const generated = result.sections.find((item) => item.title.trim().toLowerCase() === section.title.trim().toLowerCase()) ?? result.sections[index]; return generated ? { ...section, lyrics: generated.lyrics, emotion: generated.emotion || section.emotion, rhyme: generated.rhyme || section.rhyme, syllables: generated.syllables || section.syllables } : section; }); patchProject({ ...result.plan, title: generatedTitle, sections: filledSections }); setActiveId(filledSections[0]?.id ?? ''); setShowStructureGenerator(false); setMessage(`《${generatedTitle}》生成成功；点击顶部歌名可继续修改`); } catch (error) { setMessage(error instanceof Error ? error.message : '生成失败'); } finally { setStructureLoading(false); } };
  ...
  return <div className="relative flex h-full min-h-0 flex-col bg-background text-foreground">
    ...
    {showStructureGenerator && <div className="absolute inset-0 z-[70] flex items-center justify-center bg-black/40 p-6" onMouseDown={(event) => { if (event.target === event.currentTarget && !structureLoading && !promptOptimizing) setShowStructureGenerator(false); }}><div role="dialog" aria-modal="true" aria-label="AI 生成歌曲结构" className="w-full max-w-2xl rounded-2xl border bg-background p-6 shadow-2xl"><h2 className="text-base font-semibold">从创作提示开始</h2><p className="mt-1 text-xs text-muted-foreground">描述主题、风格、情绪和故事。可以先让 AI 优化表达，再规划结构或一次完成歌词。</p><textarea autoFocus value={structurePrompt} disabled={promptOptimizing} onChange={(event) => setStructurePrompt(event.target.value)} placeholder="例如：一首三分钟的华语流行情歌。写少年在夏末车站告别，主歌克制，副歌出现两次，Bridge 时间跳跃，结尾留白。不要模仿或复用任何已有歌词。" className="mt-4 min-h-36 w-full resize-y rounded-xl border bg-card p-4 text-sm leading-6 outline-none focus:border-violet-500 disabled:opacity-60" /><div className="mt-2 flex items-center justify-between gap-3"><span className="text-[11px] text-muted-foreground">AI 会保留原意，补全结构、情绪推进与创作约束。</span><Button variant="outline" size="sm" disabled={structureLoading || promptOptimizing || !structurePrompt.trim()} onClick={() => void handleOptimizePrompt()}>{promptOptimizing ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <Sparkles className="mr-1 h-3.5 w-3.5" />}{promptOptimizing ? '优化中…' : 'AI 优化'}</Button></div><div className="mt-3 rounded-lg bg-muted/40 p-3 text-[11px] text-muted-foreground">“仅生成结构”会创建空白段落；“结构并生成歌词”会自动完成第二步。两种方式都可以继续手动调整。</div><div className="mt-5 flex flex-wrap justify-end gap-2"><Button variant="outline" disabled={structureLoading || promptOptimizing} onClick={() => setShowStructureGenerator(false)}>取消</Button><Button variant="outline" disabled={structureLoading || promptOptimizing || !structurePrompt.trim()} onClick={() => void handleGenerateStructure(false)}>仅生成结构</Button><Button disabled={structureLoading || promptOptimizing || !structurePrompt.trim()} onClick={() => void handleGenerateStructure(true)}>{structureLoading ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Sparkles className="mr-1 h-4 w-4" />}结构并生成歌词</Button></div></div></div>}
