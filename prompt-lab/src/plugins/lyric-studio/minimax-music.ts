...
function hexToObjectUrl(hex: string, format: string): string {
...
export async function generateMiniMaxMusic(settings: MiniMaxMusicSettings, project: LyricProject, signal?: AbortSignal): Promise<MiniMaxMusicResult> {
  ...
  const response = await fetch('https://api.minimaxi.com/v1/music_generation', {
    ...
    body: JSON.stringify({ model: settings.model, prompt: settings.prompt.trim(), lyrics, output_format: 'url', audio_setting: { sample_rate: settings.sampleRate, bitrate: settings.bitrate, format: settings.format }, aigc_watermark: false, lyrics_optimizer: false }),
  });
  ...
  return { audioUrl: /^https?:\/\//.test(audio) ? audio : hexToObjectUrl(audio, settings.format), durationMs: result.extra_info?.music_duration, sampleRate: result.extra_info?.music_sample_rate, bitrate: result.extra_info?.bitrate, size: result.extra_info?.music_size, traceId: result.trace_id };
}
