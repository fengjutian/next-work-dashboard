...
export function makeWereadMarkdown(books: WereadExportBook[], incremental = false): string {
  ...
  for (const book of books) {
    ...
    for (const highlight of book.highlights) {
      ...
      if (chapterName !== previousChapter) { lines.push(`### ${chapterName}`, ''); previousChapter = chapterName; }
    ...
    if (book.reviews.length) { lines.push('### 想法与点评', ''); }
    for (const item of book.reviews) {
      const review = objectOf(item.review || item);
      const abstract = String(review.abstract || '').trim(); const content = String(review.content || '').trim();
