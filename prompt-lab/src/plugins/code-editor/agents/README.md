...
- `agent-sessions.ts`、`../hooks/useAgentSessions.ts`：会话模型与持久化。
- `agent-edit-scope.ts`：工作区、目录和文件作用范围。
- `ai-context.ts`、`ai-token-budget.ts`：上下文与 Token 预算。
- `ai-conversation.ts`、`../hooks/useAiSessionState.ts`：对话、候选和请求状态。
- `../hooks/useAiEditGeneration.ts`：Agent 任务生成与主进程任务调用。
- `../hooks/useAiProposalReview.ts`、`ai-proposal-summary.ts`：候选审阅和变更摘要。
