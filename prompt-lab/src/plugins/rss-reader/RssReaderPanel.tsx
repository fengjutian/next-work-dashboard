...
export const RssReaderPanel: React.FC = () => {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setState(next);
      const failures = next.subscriptions.filter((feed) => feed.error).map((feed) => feed.title);
  ...
  return <div className="flex h-full min-h-0 bg-card text-foreground">
    ...
    <main className="flex-1 min-w-0 overflow-auto bg-background/30">{current ? <article className="max-w-[780px] mx-auto px-10 py-12"><header className="pb-7 mb-8 border-b"><div className="flex items-start justify-between gap-6"><div><p className="text-xs font-medium text-primary mb-3">{current.feedTitle}</p><h1 className="text-3xl font-semibold leading-[1.3] tracking-tight">{current.title}</h1><p className="text-sm text-muted-foreground mt-4">{[current.author, dateLabel(current.publishedAt)].filter(Boolean).join(' · ')}</p></div><Button variant="ghost" size="sm" onClick={() => patchArticle(current, { starred: !current.starred })} title={current.starred ? '取消收藏' : '收藏'}><Star className={`h-5 w-5 ${current.starred ? 'text-warning fill-current' : ''}`} /></Button></div></header>{fullTexts[`${current.feedId}:${current.id}`]?.markdown ? <div className="prose prose-slate dark:prose-invert max-w-none prose-p:text-[16px] prose-p:leading-8 prose-p:my-5 prose-headings:mt-9 prose-headings:mb-4 prose-a:text-primary prose-blockquote:border-primary/40 prose-blockquote:text-muted-foreground"><ReactMarkdown remarkPlugins={[remarkGfm]} components={{ a: ({ href, children }) => <a href={href} onClick={(event) => { event.preventDefault(); if (href) void window.electronAPI.shell.openExternal(href); }}>{children}</a>, img: () => null }}>{fullTexts[`${current.feedId}:${current.id}`].markdown}</ReactMarkdown></div> : <div className="text-[16px] leading-8 whitespace-pre-wrap text-foreground/80">{current.description || '该订阅源没有提供摘要。'}</div>}{fullTexts[`${current.feedId}:${current.id}`] && <p className="mt-8 pt-4 border-t text-xs text-muted-foreground">正文已离线保存 · {fullTexts[`${current.feedId}:${current.id}`].wordCount} 词</p>}<div className="mt-8 flex gap-2">{current.link && !fullTexts[`${current.feedId}:${current.id}`] && <Button variant="outline" disabled={extracting} onClick={() => void loadFullText(current)}>{extracting ? <Loader2 className="mr-2 h-4 w-4" /> : null}提取并离线保存全文</Button>}{current.link && <Button onClick={() => window.electronAPI.shell.openExternal(current.link)}>阅读原文 <ExternalLink className="ml-2 h-4 w-4" /></Button>}</div></article> : <div className="h-full flex items-center justify-center text-sm text-muted-foreground">选择一篇文章开始阅读</div>}</main>
  </div>;
};
