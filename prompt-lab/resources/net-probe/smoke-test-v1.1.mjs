...
lines.on('line', (line) => {
  ...
  if (obj.type === 'ready') {
    send({ type: 'add_target', id: 'icmp1', target: '1.1.1.1', probe: 'icmp', interval_ms: 1000 });
    send({ type: 'add_target', id: 'tcp1', target: '1.1.1.1', probe: 'tcp', interval_ms: 2000, options: { port: 443 } });
    send({ type: 'add_target', id: 'dns1', target: 'example.com', probe: 'dns', interval_ms: 2000, options: { record: 'A' } });
    send({ type: 'add_target', id: 'http1', target: 'http://example.com', probe: 'http', interval_ms: 3000 });
    return;
  }
  if (obj.type === 'probe_result') {
    const payloadKeys = obj.payload ? Object.keys(obj.payload).slice(0, 5).join(',') : 'none';
    console.log(`[${obj.id}] ${obj.probe} success=${obj.success} latency=${obj.latency_ms ? obj.latency_ms.toFixed(1) : '-'}ms payload_keys=${payloadKeys}${obj.error ? ' err=' + obj.error : ''}`);
    results.push(obj);
    if (results.length >= 8) {
      send({ type: 'shutdown' });
