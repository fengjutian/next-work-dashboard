...
import { extractWereadWords } from '../core/analysis';
import { useWereadAdapter } from './context';
import type { WereadReviewState } from '../core/types';
...
export const WereadKnowledgeInsights = memo(function WereadKnowledgeInsights({ books, theme, onSelectBook }: { books: AnalyticsBook[]; theme: ThemePalette; onSelectBook: (bookId: string) => void }) {
  ...
  useEffect(() => { try { setReviewStates(tasks.loadReviewStates()); } catch { setReviewStates([]); } }, [tasks]);
  ...
  async function markReviewed(bookId: string, interval: number) {
    try { const state = tasks.markReviewed(bookId, interval); setReviewStates((current) => [...current.filter((item) => item.bookId !== bookId), state]); await tasks.flush(); setReviewError(''); }
