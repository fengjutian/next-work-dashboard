...
import type { WereadRecommendItem } from './adapter';
...
export const WereadAIRecommend = memo(function WereadAIRecommend({ books }: { books: AnalyticsBook[] }) {
  ...
  const [recommendations, setRecommendations] = useState<WereadRecommendItem[]>([]);
  ...
  const grouped = useMemo(() => {
    const groups: Record<string, WereadRecommendItem[]> = { same_author: [], similar: [], opposite: [] };
  ...
  async function generate() {
    ...
    try {
      const result = await api.wereadAiRecommend({ baseUrl: aiApi.baseUrl, apiKey: aiApi.apiKey, model: aiApi.model, books: digest });
