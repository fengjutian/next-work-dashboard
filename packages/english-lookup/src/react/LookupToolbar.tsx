...
import type { LookupHistoryItem } from "../core/types";
...
export function LookupToolbar(props: Props) {
  ...
  return (
    <section className="mx-auto mb-5 w-full max-w-4xl rounded-b-xl border border-t-0 bg-card p-3 shadow-sm sm:p-4">
      {props.history.length > 0 && (
        <div className="mb-3 flex flex-wrap items-center gap-2">
          <span className="text-[11px] text-muted-foreground">最近查询</span>
          {props.history.slice(0, 5).map((item) => (
            <button
              key={item.query}
              onClick={() => props.onHistory(item.query)}
              className="max-w-32 truncate rounded-full bg-muted px-2.5 py-1 text-[11px] hover:bg-accent"
            >
              {item.query}
            </button>
          ))}
      ...
      <form onSubmit={submit} className="flex flex-col gap-2 sm:flex-row">
        <div className="relative min-w-0 flex-1">
          ...
          <input
            ...
            className="h-10 w-full rounded-lg border bg-background pl-10 pr-3 text-sm outline-none focus:ring-2 focus:ring-primary/30"
          />
        </div>
        <Button disabled={props.loading} type="submit">
          {props.loading ? <Loader2 className="mr-2 h-4 w-4" /> : <Sparkles className="mr-2 h-4 w-4" />}
          AI 查询
        </Button>
      </form>
      <div className="mt-2 flex flex-wrap items-center gap-2 border-t pt-2">
        <span className="text-[11px] text-muted-foreground">语境目标</span>
        <input
          value={props.targetWord}
          onChange={(event) => props.onTarget(event.target.value)}
          placeholder="可选，例如 run into"
